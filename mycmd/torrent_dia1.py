...
sca = ['','1일', '3일', '7일', '1개월', '3개월']
bo_table = [ '', 'torrent_movieko','torrent_movieov', 'torrent_drama' , 'torrent_tvend'
           'torrent_music', 'torrent_ftv', 'torrent_kids']

#파일명에 검색일자시간을 넣기위한 내용
now = time.localtime()
s = '%03d-%02d-%02d-%02d-%02d' % (now.tm_year, now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min)

fx_name = 'torrent_dia_'+s+sca[int(scainput)]+bo_table[int(boinput)]+'.xls'
fc_name = 'torrent_dia_'+s+sca[int(scainput)]+bo_table[int(boinput)]+'.csv'
...
#    try :
    ...
    try :
        title2 = soup.select_one('span#TITLE_google')
        if title2 :
            title2 = title2.text
        else :
            title2 = '제목 없음'
        explain = soup.select_one('p.desc')
        if explain :
